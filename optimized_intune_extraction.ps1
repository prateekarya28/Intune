#***************************************************************
#  AAD intune auth
#   v4 delegated - OPTIMIZED VERSION
# check creds with : rundll32.exe keymgr.dll,KRShowKeyMgr
# Based on working script with minimal improvements for reliability
#***************************************************************
...
#log
$Logfile = "$basedir\intune_optimized.log"
...
# Function to safely export JSON with UTF-8 encoding
Function Export-JsonSafely {
    Param (
        [array]$Data,
        [string]$FilePath
    )
    ...
    try {
        ...
        # Convert to JSON using the same method as the working script
        $jsonContent = $Data | ConvertTo-Json -Depth 100
        
        # Save with UTF-8 encoding (same as working script but explicit)
        $jsonContent | Out-File -Path $FilePath -Encoding UTF8
        
        # Validate the JSON file
        try {
            $testJson = Get-Content -Path $FilePath -Raw | ConvertFrom-Json
            LogWrite("JSON export successful - validated [$($testJson.Count)] records")
            return $true
        }
        catch {
...
if ($response.StatusCode -eq 200) {
    ...
    if ($tokens.token_type -eq "Bearer") {
        ...
        try{
            # Use v1.0 endpoint (same as working script) - more stable than beta
            $url = 'https://graph.microsoft.com/v1.0/deviceManagement/managedDevices'
            $result = Invoke-RestMethod -uri $url -ErrorAction Stop -UserAgent $ua -headers $headers -Proxy $proxyurl -ProxyCredential $proxycred
            
            # Initialize array exactly like the working script
            $machines = @()
        ...
        # Handle pagination exactly like the working script
        if ($result.'@odata.nextLink') {
            do {
                try{
                    ...
                    $ex = $_.Exception
                    LogWrite("[ERROR][$($ex.Message)]")
                    # Continue with partial data instead of breaking completely
                }
            } until (
        ...
        if ($($machines.Count) -ge $expected_count) {
            ...
            # Use the safe export function with validation
            $exportSuccess = Export-JsonSafely -Data $machines -FilePath "$basedir\machines.json"
            ...
            if ($exportSuccess) {
                ...
                $summary = @{
                    ...
                    Status = "Success"
                    ApiEndpoint = "v1.0"
                    Note = "Based on proven working script pattern"
                }
             ...
             $exportSuccess = Export-JsonSafely -Data $machines -FilePath "$basedir\machines_partial.json"
             if ($exportSuccess) {
